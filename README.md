...
Remove the binary and the data directory:

```bash
rm /usr/local/bin/lvm
rm -rf ~/.lvm
```

If you added `eval "$(lvm env)"` and `eval "$(lvm hook)"` to your shell config, remove those lines as well.
...
`.nvmrc` is read for Node.js when no `.lvmrc` exists. Supported syntax:

|  Syntax | Example `.nvmrc` | Resolves To |
|  ------ | ----------------- | ----------- |
|  Exact version | `20.14.0` | That version |
|  Partial version | `22` | Latest 22.x.x |
|  LTS | `lts/*` or `--lts` | Latest LTS |
|  LTS codename | `lts/iron` | Specific LTS line |
|  Latest LTS offset | `lts/-1` | Second-latest LTS major |
|  Latest stable | `node` | Latest non-LTS release |

If `.lvmrc` and `.nvmrc` both exist, `.lvmrc` takes priority — `.nvmrc` is only used as a fallback.
...
| -------- | ------- | ----------- |
| `LVM_NODE_MIRROR` | `https://nodejs.org/dist` | Node.js download mirror |
| `LVM_GO_MIRROR` | `https://go.dev/dl` | Go download mirror |
| `LVM_JAVA_MIRROR` | `https://api.adoptium.net/v3` | Java (Eclipse Temurin) API base URL |
| `LVM_PYTHON_MIRROR` | `https://github.com/astral-sh/python-build-standalone/releases/download` | Python build-standalone download base |
| `LVM_PYTHON_TAG` | `20260623` | Python build-standalone release tag |
| `LVM_DART_MIRROR` | `https://storage.googleapis.com/dart-archive` | Dart SDK download mirror |
| `LVM_FLUTTER_MIRROR` | `https://storage.googleapis.com/flutter_infra_release/releases` | Flutter SDK download mirror |
| `LVM_KOTLIN_MIRROR` | `https://github.com/JetBrains/kotlin/releases/download` | Kotlin compiler download mirror |
| `LVM_RUST_MIRROR` | `https://static.rust-lang.org/dist` | Rust toolchain download mirror |
| `XDG_DATA_HOME` | `~/.local/share` (fallback: `~/.lvm`) | Override LVM data directory (`$XDG_DATA_HOME/lvm`) |

Set any of these in your environment to use a custom mirror or cache directory. Example:

```bash
# Use a Node.js mirror in China
export LVM_NODE_MIRROR=https://npmmirror.com/mirrors/node

# Store LVM data in a custom location
export XDG_DATA_HOME=/data/lvm
```
