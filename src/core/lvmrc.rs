...
/// Walk up from cwd to find an rc file with the given name
...
/// Walk up from cwd to find a .lvmrc file
pub fn find_lvmrc() -> Option<PathBuf> {
    find_rc_file(config::LVM_FILENAME)
}

/// Parse .lvmrc content, returns language to version mapping
/// Format:
///   node=20.14.0
///   go=1.22.3
///   # Comment lines
...
/// Read a specific language version from .lvmrc
...
/// Write or update a language version in .lvmrc
/// Preserves comments, blank lines, and existing entry order
/// Creates .lvmrc in cwd if it does not exist
