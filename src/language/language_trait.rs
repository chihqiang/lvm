...
pub trait Language: Send + Sync {
    ...
    fn lvm_dir(&self) -> PathBuf {
        config::lvm_home_cached().join(self.subdir_name())
    }

    fn current_link(&self) -> PathBuf {
        config::lvm_home_cached()
    ...
    /// Check if a version is already installed and report if so.
    /// Returns `true` when the caller should skip installation.
    fn skip_if_installed(&self, resolved: &str) -> Result<bool> {
        let version_dir = self.version_dir(resolved);
        if self.is_installed(&version_dir) {
            crate::core::report::report(format!(
