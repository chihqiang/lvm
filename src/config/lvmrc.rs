...
pub(crate) fn find_lvmrc() -> Option<PathBuf> {
    ...
    for _ in 0..super::max_lvmrc_depth() {
        let candidate = dir.join(super::lvmrc_filename());
        if candidate.exists() {
...
pub(crate) fn parse_lvmrc(path: &Path) -> Result<HashMap<String, String>> {
    ...
    for (lineno, line) in text.lines().enumerate() {
        ...
        let (key, value) = match line.split_once('=') {
            Some(pair) => pair,
            None => bail!(".lvmrc:{}: invalid format (expected key=value)", lineno + 1),
        };
