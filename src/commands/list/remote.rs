...
/// Format a single version line for display, handling color and check marks.
fn format_version_line(version: &str, is_installed: bool, is_lts: bool, use_color: bool) -> String {
    if use_color {
        ...
        format!("{color}{version}{check}{reset}")
    } else {
        let check = if is_installed {
...
/// List remote available versions, optionally filtering to LTS only
pub(crate) fn list_remote(
        ...
        .filter(|v| !lts_only || v.contains(lts_marker))
    {
        ...
        println!(
            "{}",
            format_version_line(version, is_installed, is_lts, use_color)
        );
    }
